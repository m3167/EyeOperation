...
namespace BNG
{
    ...
    public class PlayerRotation : MonoBehaviourPunCallbacks
    {
        ...
        [Header("Input")] [Tooltip("Set to false to skip Update")]
        public bool AllowInput = true;

        [Tooltip(
            "Used to determine whether to turn left / right. This can be an X Axis on the thumbstick, for example. -1 to snap left, 1 to snap right.")]
        ...
        [Tooltip(
            "Snap rotation will rotate a fixed amount of degrees on turn. Smooth will linearly rotate the player.")]
        public RotationMechanic RotationType = RotationMechanic.Snap;

        [Header("Snap Turn Settings")] [Tooltip("How many degrees to rotate if RotationType is set to 'Snap'")]
        ...
        [Header("Smooth Turn Settings")] [Tooltip("How fast to rotate the player if RotationType is set to 'Smooth'")]
        ...
        void Update()
        {
            ...
            if (RotationType == RotationMechanic.Snap)
            {
                ...
                {
                    DoSnapRotationPlayer(xAxis);
                }
                else
                {
                    if (PhotonNetwork.IsConnectedAndReady && PhotonNetwork.IsMasterClient)
                    {
                        photonView.RPC("DoSnapRotation", RpcTarget.AllBuffered, xAxis);
                    }
                }
            }
            ...
            else if (RotationType == RotationMechanic.Smooth)
            {
                if (SceneManager.GetActiveScene().name == "RegisterScene" ||
                    SceneManager.GetActiveScene().name == "LobbyScene")
                {
                    DoSmoothRotationPlayer(xAxis);
                }
                else
                {
                    if (PhotonNetwork.IsConnectedAndReady && PhotonNetwork.IsMasterClient)
                    {
                        photonView.RPC("DoSmoothRotation", RpcTarget.AllBuffered, xAxis);
                    }
                }
            }
        ...
        public virtual void DoSnapRotation(float xInput)
        {
            ...
            if (Math.Abs(rotationAmount) > 0)
            {
                ...
                if (rotatedObj != null)
                {
                    rotatedObj.transform.rotation = Quaternion.Euler(new Vector3(rotatedObj.transform.eulerAngles.x,
                        rotatedObj.transform.eulerAngles.y + rotationAmount, rotatedObj.transform.eulerAngles.z));
                }
        ...
        public virtual void DoSnapRotationPlayer(float xInput)
        {
            // Reset rotation amount before retrieving inputs
            rotationAmount = 0;
            ...
            if (Math.Abs(rotationAmount) > 0)
            {
                ...
                transform.rotation = Quaternion.Euler(new Vector3(transform.eulerAngles.x,
                    transform.eulerAngles.y + rotationAmount, transform.eulerAngles.z));
        ...
        public virtual void DoSmoothRotationPlayer(float xInput)
        {
            // Reset rotation amount before retrieving inputs
            rotationAmount = 0;
