...
namespace BNG
{
    ...
    {
        [SerializeField] Transform tabletMaster, tabletNonMaster;
        public GameObject parentTablet;
        [SerializeField] Vector3 tabletPosition;
        [SerializeField] Quaternion tabletRotation;
        [SerializeField] Vector3 tabletScale;

        [Tooltip(
            "Transform of the local player's head to track. This will be applied to the Remote Player's Head Transform")]
        ...
        // public Vector3 tabletWindowsPosition;
        // public Quaternion tabletWindowsRotation;
        ...
        void Start()
        {
            ...
            if (photonView.IsMine)
            {
                ...
                if (PhotonNetwork.LocalPlayer.CustomProperties.TryGetValue(
                        MultiplayerVRConstants.AVATAR_SELECTION_NUMBER, out avatarSelectionNumber))
                {
        ...
        void Update()
        {
            ...
            if (!photonView.IsMine)
            {
                ...
                updateRemotePositionRotation(RemoteHeadTransform, _syncHeadStartPosition, _syncHeadEndPosition,
                    _syncHeadStartRotation, _syncHeadEndRotation, synchValue);
                updateRemotePositionRotation(RemoteLeftHandTransform, _syncLHandStartPosition, _syncLHandEndPosition,
                    _syncLHandStartRotation, _syncLHandEndRotation, synchValue);
                updateRemotePositionRotation(RemoteRightHandTransform, _syncRHandStartPosition, _syncRHandEndPosition,
                    _syncRHandStartRotation, _syncRHandEndRotation, synchValue);

                // Update animation info
                if (RemoteLeftHandAnimator)
                {
                    _syncLeftGripStart = Mathf.Lerp(_syncLeftGripStart, _syncLeftGripEnd,
                        Time.deltaTime * HandAnimationSpeed);
                    RemoteLeftHandAnimator.SetFloat("Flex", _syncLeftGripStart);
                    RemoteLeftHandAnimator.SetLayerWeight(0, 1);

                    _syncLeftPointStart = Mathf.Lerp(_syncLeftPointStart, _syncLeftPointEnd,
                        Time.deltaTime * HandAnimationSpeed);
                    RemoteLeftHandAnimator.SetLayerWeight(2, _syncLeftPointStart);

                    _syncLeftThumbStart = Mathf.Lerp(_syncLeftThumbStart, _syncLeftThumbEnd,
                        Time.deltaTime * HandAnimationSpeed);
                ...
                {
                    _syncRightGripStart = Mathf.Lerp(_syncRightGripStart, _syncRightGripEnd,
                        Time.deltaTime * HandAnimationSpeed);
                    RemoteRightHandAnimator.SetFloat("Flex", _syncRightGripStart);

                    _syncRightPointStart = Mathf.Lerp(_syncRightPointStart, _syncRightPointEnd,
                        Time.deltaTime * HandAnimationSpeed);
                    RemoteRightHandAnimator.SetLayerWeight(2, _syncRightPointStart);

                    _syncRightThumbStart = Mathf.Lerp(_syncRightThumbStart, _syncRightThumbEnd,
                        Time.deltaTime * HandAnimationSpeed);
        ...
        public void InitializeSelectedAvatarModel(int avatarSelectionNumber)
        {
            ...
            loadableAvatarModels[avatarSelectionNumber].SetActive(true);
        }


        public void AssignPlayerObjects()
        {
            tabletMaster = getChildTransformByName(gameObject.transform, "TabletMaster");
            tabletMaster.SetParent(parentTablet.transform);
            tabletMaster.transform.localPosition = tabletPosition;
            tabletMaster.transform.localRotation = tabletRotation;
            tabletMaster.transform.localScale = tabletScale;
            
            tabletNonMaster = getChildTransformByName(gameObject.transform, "TabletNotMaster");
            tabletNonMaster.SetParent(parentTablet.transform);
            tabletNonMaster.transform.localPosition = tabletPosition;
            tabletNonMaster.transform.localRotation = tabletRotation;
            tabletNonMaster.transform.localScale = tabletScale;
        ...
        public virtual bool RecentlyRequested(PhotonView view)
        {
            ...
            return requestedGrabbables != null && requestedGrabbables.ContainsKey(view.ViewID) &&
        ...
        void updateRemotePositionRotation(Transform moveTransform, Vector3 startPosition, Vector3 endPosition,
            Quaternion syncStartRotation, Quaternion syncEndRotation, float syncValue)
        {
        ...
        internal void DrawLine_Remote(int lineId, int markerIndex, float lastDrawPointX, float lastDrawPointY,
            float lastDrawPointZ, float endPositionX,
            float endPositionY, float endPositionZ, float lineWidth, float lineColorR, float lineColorG,
            float lineColorB,
            float rotationX, float rotationY, float rotationZ, float rotationW)
        {
            if (photonView.IsMine)
            {
                photonView.RPC("RPC_DrawLine", RpcTarget.AllBuffered, lineId, markerIndex, lastDrawPointX,
                    lastDrawPointY, lastDrawPointZ, endPositionX, endPositionY,
                    endPositionZ, lineWidth, lineColorR, lineColorG, lineColorB, rotationX, rotationY, rotationZ,
                    rotationW);
            }
        }
        ...
        [PunRPC]
        void RPC_DrawLine(int lineId, int markerIndex, float lastDrawPointX, float lastDrawPointY, float lastDrawPointZ,
            float endPositionX,
            float endPositionY, float endPositionZ, float lineWidth, float lineColorR, float lineColorG,
            float lineColorB,
            float rotationX, float rotationY, float rotationZ, float rotationW)
        {
            paintManager = GameObject.FindObjectOfType<PaintManager>();
            paintManager.markers[markerIndex].DrawLineViaNetwork(lineId, lastDrawPointX, lastDrawPointY, lastDrawPointZ,
        ...
        internal void ContinueDrawLine_Remote(int markerIndex, float endPositionX, float endPositionY,
            float endPositionZ, float lineWidth)
        {
            if (photonView.IsMine)
            {
                photonView.RPC("RPC_ContinueDrawLine", RpcTarget.AllBuffered, markerIndex, endPositionX, endPositionY,
                    endPositionZ, lineWidth);
            }
        }

        [PunRPC]
        void RPC_ContinueDrawLine(int markerIndex, float endPositionX, float endPositionY, float endPositionZ,
            float lineWidth)
        {
            paintManager = GameObject.FindObjectOfType<PaintManager>();
            paintManager.markers[markerIndex]
                .ContinueDrawLineViaNetwork(endPositionX, endPositionY, endPositionZ, lineWidth);
        }
